...
if (text === '/cancel' && state && state.step === 'waiting_match') {
  ...
  if (state.step === 'rate') {
    ...
    if (isNaN(rate) || rate < 0 || rate > 100) return bot.sendMessage(userId, '⚠️ درصد ریت را به صورت عدد بین 0 تا 100 وارد کن');
    if (state.type === 'rate') {
      state.rate = rate;
      state.step = 'target';
  ...
  if (state.step === 'target') {
    ...
    if (isNaN(target) || target < 0 || target > 100) return bot.sendMessage(userId, '⚠️ درصد ریت را به صورت عدد بین 0 تا 100 وارد کنید');
